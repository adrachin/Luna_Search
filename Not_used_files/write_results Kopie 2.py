...
def create_html_file(file_paths, output_file, protected_dir):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for file_path in file_paths:
            ...
            total_size_gb += size
            escaped_path = escape_js(file_path)  # Kodieren des Pfades für die Verwendung in HTML/JS
            f.write(f'<tr><td><input type="checkbox" name="project" value="{escaped_path}"></td><td>{file_name}</td><td>{size:.2f}</td>')
            f.write(f'<td><button onclick="openLuna(\'{escaped_path}\')">Open</button></td>')
            f.write(f'<td><button onclick="openInFinder(\'{escaped_path}\')">Show in Finder</button></td></tr>\n')
        f.write('</table>\n')
